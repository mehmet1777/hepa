...
export function ImageUpload({ onDataExtracted }: ImageUploadProps) {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          // Kontrast artırma (0-255 arası değerleri genişlet)
          const contrast = 40;
  ...
  const extractDataFromText = (text: string): FilterData | null => {
    ...
    if (weightCandidates.length > 0) {
      netWeight = weightCandidates[0];
      console.log('✅ Net Weight bulundu:', netWeight);
    } else {
      // Daha geniş aralık dene: 40-80
      const wideWeightCandidates = allNumbers.filter(n => n >= 40 && n <= 80).sort((a, b) => a - b);
      if (wideWeightCandidates.length > 0) {
        netWeight = wideWeightCandidates[0];
        console.log('✅ Net Weight bulundu (geniş aralık):', netWeight);
      }
    }
    ...
    // 4. Eğer length hala bulunamadıysa
    if (!length && allNumbers.length > 10) {
      ...
      if (bigNumbers.length > 0) {
        ...
        console.log('⚠️ Length tahmin edildi:', length);
      }
    }
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Görüntü ön işleme
      console.log('🎨 Görüntü ön işleniyor...');
      const processedImage = await preprocessImage(file);
      
      // Önizleme için orijinal göster
      const reader = new FileReader();
      reader.onload = (e) => {
      ...
      // OCR için işlenmiş görüntü kullan
      console.log('👁️ OCR başlıyor...');
      ...
      if (extractedData) {
        ...
        // OCR başarılı ama kullanıcı düzenleyebilsin
        setShowEditForm(true);
        setFormData({
          width: extractedData.width.toString(),
          netWeight: extractedData.netWeight.toString(),
          length: extractedData.length.toString(),
        });
      } else {
    ...
    <div className="space-y-4">
      ...
      {isProcessing && (
        <div className="flex items-center justify-center space-x-2 text-slate-600">
          ...
          <span>Etiket işleniyor ve okunuyor...</span>
        </div>
      )}
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-amber-800 mb-2">
                {formData.width || formData.netWeight || formData.length 
                  ? 'Okunan değerleri kontrol edin ve gerekirse düzeltin:' 
